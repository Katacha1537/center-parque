...
import { ArrowRight, Tag, Smile, Truck, Package } from 'lucide-react';
import { Link } from 'react-router-dom';

const iconMap: any = {
  Tag, Smile, Truck, Package
};
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {categories.map((cat, index) => {
            ...
            const IconComponent = iconMap[cat.iconName] || Tag;
            
            return (
              <Link 
                to={`/catalogo?category=${cat.title}`}
                key={index}
              >
                <motion.div
                  ...
                  viewport={{ once: true }}
                  whileHover={{ y: -10 }}
                  className={`${cat.color} rounded-2xl p-8 text-white relative overflow-hidden cursor-pointer shadow-lg group h-full`}
                >
